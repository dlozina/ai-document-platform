...
class EnhancedEmbeddingProcessor:
    ...
    def chunk_document_intelligently(self, text: str, document_id: str) -> List[Dict[str, Any]]:
        """
        ...
        while start < len(text):
            ...
            if end < len(text):
                ...
                break_points = [bp for bp in break_points if bp > start + self.chunk_size // 2]
    ...
    def store_chunk_embedding(
        ...
        try:
            ...
            raise RuntimeError(f"Failed to store chunk embedding: {str(e)}")
    ...
    def process_document_with_chunking(
        ...
        try:
            logger.info(f"Processing document {document_id} with chunking (text length: {len(text)})")
            ...
            raise RuntimeError(f"Document chunking processing failed: {str(e)}")
