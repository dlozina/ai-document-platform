...
class EmbeddingProcessor:
    ...
    def __init__(
        ...
        try:
            ...
            logger.info(
                f"Model loaded successfully. Vector dimension: {self.model.get_sentence_embedding_dimension()}"
            )
    ...
    def _ensure_collection_exists(self):
        ...
        try:
            ...
            raise RuntimeError(f"Failed to setup Qdrant collection: {str(e)}") from e
                ...
                results.append(
                    {
                        ...
                        "score": result.score,
                        "text": (
                            result.payload.get("text", "") if result.payload else ""
                        ),
                        "document_id": (
                            result.payload.get("original_document_id")
                            if result.payload
                            else None
                        ),  # Include original document ID
                        "metadata": (
                            {
                                k: v
                                for k, v in result.payload.items()
                                if k not in ["text", "original_document_id"]
                            }
                            if result.payload
                            else {}
                        ),
    ...
    def process_text_file(
        ...
        try:
            ...
            raise RuntimeError(f"Text file processing failed: {str(e)}") from e
        ...
        try:
            ...
            if vectors_config is not None:
                if hasattr(vectors_config, "size"):
                    ...
                    # Handle dict case - get size from first vector config
    ...
    def health_check(self) -> dict[str, Any]:
        ...
        try:
            self.model.encode("test", convert_to_tensor=False)
