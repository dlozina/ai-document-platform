...
class SimplifiedEmbeddingProcessor:
    ...
    def __init__(
        ...
        try:
            ...
            logger.info(
                f"Model loaded successfully. Vector dimension: {actual_vector_size}"
            )

            # Update vector size if different from expected
            if actual_vector_size != vector_size:
                logger.warning(
                    f"Model vector size ({actual_vector_size}) differs from expected ({vector_size})"
                )
            ...
            while start < len(text):
                ...
                if end < len(text):
                    ...
                    for i in range(
                        ...
                        for i in range(
                            end, max(start + self.chunk_size // 2, end - 50), -1
                        ):
                            if text[i] == " ":
                                end = i
                                break
    ...
    def process_document_with_chunking(
        ...
        try:
            ...
            for i, (chunk, embedding) in enumerate(
                ...
                # Generate valid UUID for chunk_id
                chunk_id_base = f"{document_id}_chunk_{i}"
                chunk_id_uuid = str(uuid.uuid5(uuid.NAMESPACE_DNS, chunk_id_base))

                chunk_result = {
                    "chunk_id": chunk_id_uuid,  # Use valid UUID instead of concatenated string
                    "text": chunk["text"],
                    "embedding": embedding,
            ...
            logger.info(
                f"Processed document {document_id}: {len(chunks)} chunks in {processing_time:.2f}ms"
            )
