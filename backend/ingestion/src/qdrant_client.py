...
class QdrantManager:
    ...
    def __init__(
        ...
        if not QDRANT_AVAILABLE:
            raise RuntimeError("Qdrant client not available. Install qdrant-client package.")
    ...
    def store_embedding(
        ...
        try:
            ...
            logger.info(f"Stored embedding for document {document_id} with point ID {point_id}")
            return point_id
    ...
    def store_chunked_embeddings(
        ...
        try:
            ...
            logger.info(f"Stored {len(chunks)} chunked embeddings for document {document_id}")
            return point_ids
            
        except Exception as e:
            logger.error(f"Failed to store chunked embeddings for document {document_id}: {e}")
            raise
...
def get_qdrant_manager() -> QdrantManager:
    ...
    if _qdrant_manager is None:
        raise RuntimeError("Qdrant manager not initialized. Call initialize_qdrant_manager() first.")
