...
from .database import ProcessingJob, get_db_manager
...
class ProcessingPipeline:
    ...
    async def process_document(
        ...
        if pipeline_config.get("enable_embedding", False):
            embedding_job = await self._start_embedding_processing(
            ...
            self.db_manager.update_document(
                ...
                document_id,
                {
                    ...
                    "ocr_status": (
                        ...
                        if pipeline_config.get("enable_ocr")
                        else None
                    ),
                    "ner_status": (
                        ProcessingStatus.PENDING
                        if pipeline_config.get("enable_ner")
                        else None
                    ),
                    "embedding_status": (
                        ProcessingStatus.PENDING
                        if pipeline_config.get("enable_embedding")
                        else None
                    ),
                },
            )
    ...
    async def _start_ocr_processing(
        ...
        try:
            ...
            with self.db_manager.get_session() as session:
                job = self.db_manager.create_processing_job(
                    session,
                    {
                        "id": job_id,
                        "document_id": document_id,
    ...
    async def _process_ocr_async(
        ...
        try:
            # Update job status
            with self.db_manager.get_session() as session:
                self.db_manager.update_processing_job(
                    session,
                    job_id,
                    {
    ...
    async def _start_ner_processing(
        ...
        try:
            ...
            with self.db_manager.get_session() as session:
                ...
                if not document or not document.ocr_text:
                    logger.warning(
                        f"No OCR text available for NER processing: {document_id}"
                    )
                    return None
            ...
            asyncio.create_task(
                self._process_ner_async(
                    job_id, document_id, tenant_id, document.ocr_text
                )
            )
    ...
    async def _process_ner_async(
        self, job_id: str, document_id: str, tenant_id: str, text: str
    ):
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                response = await client.post(
                    f"{self.settings.ner_service_url}/extract", json=payload
                )
                ...
                if response.status_code == 200:
                    ...
                    raise Exception(
                        f"NER service returned status {response.status_code}: {response.text}"
                    )
            ...
            with self.db_manager.get_session() as session:
                self.db_manager.update_document(
                    session, document_id, {"ner_status": ProcessingStatus.FAILED}
                )

    async def _start_embedding_processing(
        self, document_id: str, tenant_id: str
    ) -> dict[str, Any | None]:
        ...
        try:
            ...
            with self.db_manager.get_session() as session:
                ...
                if not document or not document.ocr_text:
                    logger.warning(
                        f"No OCR text available for embedding processing: {document_id}"
                    )
                    return None
    ...
    async def _process_embedding_async(
        self, job_id: str, document_id: str, tenant_id: str, text: str
    ):
                ...
                if response.status_code == 200:
                    ...
                    with self.db_manager.get_session() as session:
                        self.db_manager.update_document(
                            ...
                            document_id,
                            {
                                "embedding_vector": embedding_result.get(
                                    "embedding", []
                                ),
    ...
    def check_pipeline_completion(self, document_id: str) -> bool:
        ...
        with self.db_manager.get_session() as session:
            ...
            return all(
                status == ProcessingStatus.COMPLETED for status in non_none_statuses
            )
