...
file_validator = FileValidator(
    settings.max_file_size_bytes, settings.supported_file_types
)
...
SERVICE_UPTIME = Gauge("ingestion_service_uptime_seconds", "Service uptime in seconds")
...
async def metrics_middleware(request: Request, call_next):
    ...
    REQUEST_DURATION.labels(
        method=request.method, endpoint=request.url.path, tenant_id=tenant_id
    ).observe(duration)
...
def get_tenant_id(tenant_id: str | None = Header(None, alias="X-Tenant-ID")) -> str:
...
def parse_pipeline_config(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Invalid pipeline_config JSON: {str(e)}"
        ) from e
...
async def test_upload(
    ...
    pipeline_config_json: str | None = Form(
        None, description="Processing pipeline configuration as JSON string"
    ),
):
    ...
    try:
        ...
        return {
            ...
            "tenant_id": tenant_id,
            "pipeline_config": (
                pipeline_config.model_dump() if pipeline_config else None
            ),
...
async def upload_file(
    ...
    tags: list[str | None] = Query(None, description="Document tags"),
    description: str | None = Query(None, description="Document description"),
    retention_days: int | None = Query(None, description="Retention period in days"),
    ...
    **Note:** The pipeline_config_json should be sent as a form field containing JSON string.
    ...
    try:
        ...
        if existing_doc and existing_doc.tenant_id == tenant_id:
            raise HTTPException(
                status_code=409, detail="File with identical content already exists"
            )
        ...
        if not quota_result["has_quota"]:
            raise HTTPException(status_code=413, detail="Tenant storage quota exceeded")
        ...
        logger.info(
            f"Queued OCR processing task {ocr_task.id} for document {document_id}"
        )
        ...
        UPLOAD_COUNT.labels(
            tenant_id=tenant_id, file_type=file_type, status="success"
        ).inc()

        UPLOAD_DURATION.labels(tenant_id=tenant_id, file_type=file_type).observe(
            processing_time_ms / 1000.0
        )

        UPLOAD_SIZE.labels(tenant_id=tenant_id, file_type=file_type).observe(file_size)
        ...
        UPLOAD_COUNT.labels(
            tenant_id=tenant_id, file_type="unknown", status="error"
        ).inc()
    ...
    for file in files:
        try:
            ...
            if not upload_success:
                failed_uploads += 1
                errors.append(
                    {"filename": file.filename, "error": "Failed to upload to storage"}
                )
                continue
...
async def get_processing_status(
    ...
    **Returns:**
    ...
    return ProcessingJobResponse(
        ...
        completed_at=(
            document.updated_at if document.processing_status == "completed" else None
        ),
    )
...
async def update_document(
    ...
    **Returns:**
    ...
    if update_request.retention_days is not None:
        update_data["retention_date"] = calculate_retention_date(
            update_request.retention_days
        )
...
async def get_tenant_quota(tenant_id: str, db_session=Depends(get_db)):
    """
    ...
    **Returns:**
    ...
    if not quota:
        ...
        quota = db_manager.create_tenant_quota(
            db_session,
            tenant_id,
            {
