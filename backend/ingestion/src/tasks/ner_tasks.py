...
def process_document_ner(self, document_id: str, tenant_id: str) -> dict[str, Any]:
    """
    ...
    try:
        ...
        with db_manager.get_session() as session:
            ...
            db_manager.update_document(
                session, document_id, {"ner_status": ProcessingStatus.PROCESSING}
            )
        ...
        async def call_ner_service():
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                response = await client.post(
                    f"{settings.ner_service_url}/extract", json=payload
                )
        ...
        logger.error(
            f"NER processing failed for document {document_id}: {exc}", exc_info=True
        )
        ...
        # Retry logic
        try:
            retry_countdown = RETRY_POLICIES["ner"]["countdown"] * (
            ...
            logger.info(
                f"Retrying NER processing for document {document_id} in {countdown} seconds"
            )
            raise self.retry(countdown=countdown, exc=exc)

        except MaxRetriesExceededError:
            logger.error(
                f"Max retries exceeded for NER processing of document {document_id}"
            )
...
def check_processing_completion(document_id: str, tenant_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        with db_manager.get_session() as session:
            ...
            ner_complete = (
                document.ner_status == ProcessingStatus.COMPLETED
                or document.ner_status is None
            )
            embedding_complete = (
                document.embedding_status == ProcessingStatus.COMPLETED
                or document.embedding_status is None
            )

            # Check if any processing failed
            any_failed = (
                document.ocr_status == ProcessingStatus.FAILED
                or document.ner_status == ProcessingStatus.FAILED
                or document.embedding_status == ProcessingStatus.FAILED
            )
