...
from ..database import Document, ProcessingJob, get_db_manager
...
def cleanup_failed_processing(document_id: str, tenant_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error(
            f"Failed to cleanup failed processing for document {document_id}: {exc}"
        )
        return {"document_id": document_id, "status": "error", "error": str(exc)}
...
def generate_processing_report(
    ...
    try:
        ...
        with db_manager.get_session() as session:
            ...
            completed_documents = len(
                [
                    d
                    for d in documents
                    if d.processing_status == ProcessingStatus.COMPLETED
                ]
            )
            failed_documents = len(
                [d for d in documents if d.processing_status == ProcessingStatus.FAILED]
            )
            processing_documents = len(
                [
                    d
                    for d in documents
                    if d.processing_status == ProcessingStatus.PROCESSING
                ]
            )
            ...
            logger.info(
                f"Generated processing report for tenant {tenant_id}: {completed_documents}/{total_documents} completed"
            )
        ...
        logger.error(
            f"Failed to generate processing report for tenant {tenant_id}: {exc}"
        )
        return {"tenant_id": tenant_id, "status": "error", "error": str(exc)}
