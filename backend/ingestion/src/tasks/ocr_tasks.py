    ...
    try:
        ...
        with db_manager.get_session() as session:
            ...
            db_manager.update_document(session, document_id, {
                "ocr_status": ProcessingStatus.PROCESSING
            })
        ...
        logger.info(f"Downloaded file {filename} ({len(file_content)} bytes) for OCR processing")
        ...
        logger.error(f"OCR processing failed for document {document_id}: {exc}", exc_info=True)
        ...
        # Retry logic
        try:
            retry_countdown = RETRY_POLICIES["ocr"]["countdown"] * (2 ** self.request.retries)
            max_countdown = RETRY_POLICIES["ocr"]["max_countdown"]
            countdown = min(retry_countdown, max_countdown)
            
            logger.info(f"Retrying OCR processing for document {document_id} in {countdown} seconds")
            raise self.retry(countdown=countdown, exc=exc)
            
        except MaxRetriesExceededError:
            logger.error(f"Max retries exceeded for OCR processing of document {document_id}")
...
def trigger_next_processing(document_id: str, tenant_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Failed to trigger next processing for document {document_id}: {exc}")
        return {
            "document_id": document_id,
            "status": "failed",
            "error": str(exc)
...
def move_to_dead_letter(
    ...
    Returns:
        Result dictionary
    """
    logger.error(f"Moving document {document_id} to dead letter queue. Task: {task_type}, Error: {error_message}")
    ...
    try:
        ...
        logger.error(f"Failed to move document {document_id} to dead letter queue: {exc}")
        return {
            "document_id": document_id,
            "status": "failed",
            "error": str(exc)
