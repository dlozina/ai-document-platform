...
def test_language_support():
    ...
    try:
        payload = {"text": english_text, "language": "en", "include_confidence": True}
    ...
    try:
        payload = {"text": croatian_text, "language": "hr", "include_confidence": True}
        response = requests.post(f"{BASE_URL}/extract", json=payload)
        if response.status_code == 200:
            data = response.json()
            print("✅ Croatian processing successful!")
            print(f"   Model used: {data['spacy_model_used']}")
            print(f"   Language: {data['language']}")
            print(f"   Entities found: {data['entity_count']}")
            for entity in data["entities"]:
