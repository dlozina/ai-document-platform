...
ner_processor: NERProcessor | None = None
...
async def extract_entities(
    ...
    **Returns:**
    ...
    try:
        ...
        logger.info(
            f"Successfully processed text: {result['entity_count']} entities found"
        )
...
async def process_text_background(
    ...
    try:
        ...
        if not ner_processor.is_language_supported(request.language):
            logger.error(
                f"Unsupported language in background job {job_id}: {request.language}"
            )
            return
...
async def get_entity_statistics(
    request: NERRequest, tenant_id: str | None = Header(None, alias="X-Tenant-ID")
):
    """
...
async def visualize_entities(
    ...
    **Returns:**
    ...
    try:
        ...
        html = ner_processor.visualize_entities(
            request.text, result["entities"], request.language
        )
...
async def get_supported_languages():
    ...
    **Returns:**
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get language information: {str(e)}"
        ) from e
...
if __name__ == "__main__":
    uvicorn.run("main:app", host="0.0.0.0", port=8001, reload=True, log_level="info")  # nosec B104 - Required for containerized microservice
