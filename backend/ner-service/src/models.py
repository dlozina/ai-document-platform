...
class HealthResponse(BaseModel):
    ...
    spacy_models_available: dict[str, bool] = Field(
        ..., description="Available Spacy models"
    )
...
class NERRequest(BaseModel):
    ...
    text: str = Field(
        ..., description="Text to process", min_length=1, max_length=1000000
    )
    ...
    language: str = Field(
        "en", description="Language code (en, hr)", pattern="^(en|hr)$"
    )
...
class BatchNERRequest(BaseModel):
    ...
    texts: list[str] = Field(
        ..., description="List of texts to process", min_items=1, max_items=100
    )
    entity_types: list[str | None] = Field(
        None, description="Specific entity types to extract"
    )
...
class NERStatsResponse(BaseModel):
    ...
    entity_types: list[EntityStats] = Field(
        ..., description="Statistics by entity type"
    )
    most_common_entities: list[dict[str, Any]] = Field(
        ..., description="Most frequently detected entities"
    )
