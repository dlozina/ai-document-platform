...
class OCRProcessor:
    ...
    def process_file(
        self, file_content: bytes, filename: str, force_ocr: bool = False
    ) -> dict[str, any]:
        """
    ...
    def _process_pdf(
        self, pdf_content: bytes, force_ocr: bool = False
    ) -> dict[str, any]:
        """
        ...
        if not force_ocr:
            ...
            logger.info(
                "Native PDF extraction yielded minimal text, falling back to OCR"
            )
    ...
    def _extract_pdf_text_via_ocr(self, pdf_content: bytes) -> dict[str, any]:
        ...
        images = convert_from_bytes(pdf_content, dpi=self.dpi, fmt="png")
