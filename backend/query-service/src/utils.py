...
def format_search_result(
    result: dict[str, Any], max_text_length: int = 500
) -> dict[str, Any]:
...
def calculate_confidence_score(
    similarity_scores: list[float], entity_matches: int = 0, keyword_matches: int = 0
) -> float:
    ...
    if "start" in date_range and date_range["start"]:
        try:
            if isinstance(date_range["start"], str):
                parsed["start"] = datetime.fromisoformat(
    ...
    if "end" in date_range and date_range["end"]:
        try:
            if isinstance(date_range["end"], str):
                parsed["end"] = datetime.fromisoformat(
