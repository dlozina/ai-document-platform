...
class QueryProcessor:
    ...
    def _initialize_nlp(self):
        ...
        try:
            ...
            logger.warning(
                f"Failed to load spaCy model: {e}. NER features will be limited."
            )
            self.nlp = None
    ...
    def _initialize_llm(self):
        ...
        try:
            ...
            if not api_key:
                logger.warning(
                    f"No API key found for {provider}. LLM features will be disabled."
                )
                return

            if not model:
                logger.warning(
                    f"No model specified for {provider}. LLM features will be disabled."
                )
                return
            ...
            logger.warning(
                f"Failed to initialize LLM manager: {e}. LLM features will be disabled."
            )
            self.llm_manager = None
    ...
    def _build_qdrant_filter(
        self, filter_params: dict[str, Any | None], tenant_id: str | None = None
    ) -> Filter | None:
        ...
        if filter_params:
            for key, value in filter_params.items():
                ...
                if key == "content_type" and value:
                    conditions.append(
                        FieldCondition(
                            key="content_type", match=MatchValue(value=value)
                        )
                    )
                elif key == "file_type" and value:
                    conditions.append(
                        FieldCondition(key="file_type", match=MatchValue(value=value))
                    )
                elif key == "processing_status" and value:
                    conditions.append(
                        FieldCondition(
                            key="processing_status", match=MatchValue(value=value)
                        )
                    )
                elif key == "created_by" and value:
                    conditions.append(
                        FieldCondition(key="created_by", match=MatchValue(value=value))
                    )
                elif key == "file_size_min" and value:
                    conditions.append(
                        FieldCondition(key="file_size_bytes", range=Range(gte=value))
                    )
                elif key == "file_size_max" and value:
                    conditions.append(
                        FieldCondition(key="file_size_bytes", range=Range(lte=value))
                    )
        ...
        try:
            ...
            for result in search_results:
                ...
                if chunk_id is not None:
                    ...
                    logger.warning(
                        f"Skipping non-chunked document: {result.payload.get('document_id')}"
                    )
                    continue
    ...
    def collect_entities_from_results(
        ...
        for result in results:
            ...
            for entity in doc_entities:
                ...
                entity_key = (
                    f"{entity.get('text', '').lower()}_{entity.get('label', '')}"
                )
    ...
    def filter_by_entities(
        ...
        entity_text: list[str | None] = None,
    ) -> list[dict[str, Any]]:
        ...
        for result in results:
            ...
            if entity_text:
                has_matching_text = any(
                    any(
                        entity.get("text", "").lower() == text.lower()
                        for entity in doc_entities
                    )
                ...
                for d_entity in doc_entities:
                    ...
                    if q_text in d_text or d_text in q_text:
                        matching_entities.append(
                            {"question_entity": q_entity, "document_entity": d_entity}
                        )
    ...
    def _extract_contextual_answer(
        ...
        try:
            ...
            if entity_label in ["PERSON", "PER"]:
                return self._extract_person_context(
                    entity_text, entity_sentences, question_lower
                )
            elif entity_label in ["GPE", "LOCATION", "LOC"]:
                return self._extract_location_context(
                    entity_text, entity_sentences, question_lower
                )
            elif entity_label in ["ORG", "ORGANIZATION"]:
                return self._extract_organization_context(
                    entity_text, entity_sentences, question_lower
                )
            else:
                # Generic entity extraction
                return self._extract_generic_context(
                    entity_text, entity_sentences, question_lower
                )
    ...
    def _extract_location_context(
        self, entity_text: str, sentences: list[str], question_lower: str
    ) -> tuple[str, float]:
        ...
        for sentence in sentences:
            ...
            if entity_text.lower() in sentence_lower:
                ...
                if any(
                    ...
                    for word in ["based in", "located in", "from", "in", "at"]
                ):
    ...
    def _extract_person_context(
        ...
        for sentence in sentences:
            ...
            if entity_text.lower() in sentence_lower:
                ...
                if any(
                    word in sentence_lower
                    for word in ["i'm", "i am", "my name is", "hello"]
                ):
                    return sentence, 0.7
    ...
    def _extract_organization_context(
        self, entity_text: str, sentences: list[str], question_lower: str
    ) -> tuple[str, float]:
        ...
        for sentence in sentences:
            ...
            if entity_text.lower() in sentence_lower:
                ...
                if any(
                    word in sentence_lower
                    for word in ["worked at", "company", "startup", "firm"]
                ):
                    return sentence, 0.7
    ...
    def _extract_keyword_based_answer(
        ...
        try:
            ...
            for sentence in sentences:
                ...
                question_words = [
                    word for word in question_lower.split() if len(word) > 2
                ]  # Reduced threshold
            ...
            if relevant_sentences:
                ...
                confidence = min(
                    0.7, len(relevant_sentences) / 3.0
    ...
    async def generate_rag_answer(
        ...
        try:
            if not self.llm_manager:
                logger.warning(
                    "LLM manager not available, falling back to template response"
                )
                fallback_answer, fallback_confidence = self._generate_fallback_answer(
            ...
            (
                ...
                confidence,
                truncation_occurred,
            ) = await self.llm_manager.generate_rag_answer(
                question=question,
    ...
    def health_check(self) -> dict[str, Any]:
        ...
        except Exception:  # nosec B110 - Intentionally ignore Qdrant connection errors
            pass
