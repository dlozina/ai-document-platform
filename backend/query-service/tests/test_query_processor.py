...
class TestQueryProcessorInitialization:
    ...
    def test_initialization_without_spacy(self, mock_settings):
        ...
        with (
            ...
            patch("src.query_processor.create_engine"),
            patch(
                "src.query_processor.spacy.load",
                side_effect=Exception("spaCy not available"),
            ),
            patch("src.query_processor.CrossEncoder"),
        ):
            processor = QueryProcessor()
    ...
    def test_initialization_without_rerank(self, mock_settings):
        ...
        with (
            ...
            patch("src.query_processor.spacy.load"),
            patch(
                "src.query_processor.CrossEncoder",
                side_effect=Exception("Rerank model not available"),
            ),
        ):
            processor = QueryProcessor()
...
class TestEmbeddingGeneration:
    ...
    def test_generate_query_embedding_success(self, query_processor):
        ...
        query_processor.embedding_model.encode.assert_called_once_with(
            "test query", convert_to_tensor=False
        )
...
class TestSemanticSearch:
    ...
    def test_semantic_search_success(self, query_processor):
        ...
        results = query_processor.semantic_search(
            query="test query", top_k=10, score_threshold=0.7, tenant_id="test-tenant"
        )
    ...
    def test_semantic_search_with_filter(self, query_processor):
        ...
        filter_params = {"content_type": "application/pdf", "file_type": "pdf"}

        query_processor.qdrant_client.search.return_value = []

        query_processor.semantic_search(
            query="test query", filter_params=filter_params, tenant_id="test-tenant"
        )
...
class TestEntityFiltering:
    ...
    def test_filter_by_entities_with_labels(self, query_processor):
        ...
        results = [
            ...
            {
                "text": "Document 2",
                "metadata": {"ner_entities": [{"text": "Java", "label": "ORG"}]},
            },
        ]

        filtered = query_processor.filter_by_entities(results, entity_labels=["PERSON"])
    ...
    def test_filter_by_entities_with_text(self, query_processor):
        ...
        results = [
            {
                "text": "Document 1",
                "metadata": {"ner_entities": [{"text": "Python", "label": "ORG"}]},
            }
        ]

        filtered = query_processor.filter_by_entities(results, entity_text=["Python"])

        assert len(filtered) == 1

    def test_filter_by_entities_no_entities(self, query_processor):
        """Test filtering when documents have no entities."""
        results = [{"text": "Document 1", "metadata": {}}]
...
class TestAnswerExtraction:
    ...
    def test_extract_answer_span_success(self, query_processor):
        ...
        answer, confidence = query_processor.extract_answer_span(
...
class TestRAGGeneration:
    ...
    def test_generate_rag_answer_success(self, query_processor):
        ...
        question = "What is Python?"
        context_documents = [
            {"text": "Python is a programming language", "filename": "doc1.pdf"},
...
class TestHealthCheck:
    ...
    def test_health_check_all_healthy(self, query_processor):
        ...
        with patch("src.query_processor.text"):
            mock_conn = Mock()
            query_processor.db_engine.connect.return_value.__enter__.return_value = (
                mock_conn
            )
    ...
    def test_health_check_qdrant_unavailable(self, query_processor):
        ...
        query_processor.qdrant_client.get_collections.side_effect = Exception(
            "Connection failed"
        )
